  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        try {
          const workerCheck = await fetch('/pdf.worker.min.mjs')
          const workerText = await workerCheck.text()
          alert(
            'main thread getOrInsertComputed: ' + typeof (Map.prototype as any).getOrInsertComputed +
            '\npolyfillsApplied: ' + JSON.stringify((window as any).__polyfillsApplied) +
            '\nworker has polyfill marker: ' + workerText.includes('Auto-generated by scripts/setup-pdf-worker.js') +
            '\nworker file length: ' + workerText.length +
            '\nfirst 100 chars: ' + workerText.slice(0, 100)
          )
        } catch (diagErr: any) {
          alert('diagnostic fetch failed: ' + diagErr.message)
        }
        // END TEMP DIAGNOSTIC
